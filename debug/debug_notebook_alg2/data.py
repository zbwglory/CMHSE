...
class PrecompDataset(data.Dataset):
    ...
    def img_cap_feat_combine(self, c3d_feat, captions, cur_vid, vocab):
        ...
        caption.extend([vocab(token) for token in tokens_whole])
        caption.append(vocab('EOS'))
        caption.append(vocab('BOS'))
        caption.extend([vocab(token) for token in tokens_segment])
        caption.append(vocab('EOS'))

        return video_repre, caption
    ...
    def __getitem__(self, index):
        ...
        vocab = self.vocab
        image,caption = self.img_cap_feat_combine(c3d_feat, captions, cur_vid, vocab)
